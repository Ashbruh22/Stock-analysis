...
**BCSI ships the model + SHAP + real-time UI + multi-user infra as one open-source platform. github.com/Ashbruh22/Stock-analysis. MIT, self-hosted, no upsell. The `/api/v1/ml/predict/{ticker}` endpoint demoed above returns the JSON exactly as shown.**
